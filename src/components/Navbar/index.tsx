  ...
  return (
    <div className="fixed hidden md:flex z-[999] top-1/2 -translate-y-1/2 left-5 flex-col gap-2.5">
      <ButtonNav
        ...
        Icon={<BiHome className="min-w-[25px] min-h-[25px]" />}
        text={t("home")}
      />
      <ButtonNav
        onClick={() => scrollTo("About")}
        Icon={<BiUser className="min-w-[25px] min-h-[25px]" />}
        text={t("about")}
      />
      <ButtonNav
        onClick={() => scrollTo("LanguageTools")}
        Icon={<GiSkills className="min-w-[25px] min-h-[25px]" />}
      ...
      <div className={"relative"}>
        <button
          onClick={() => setOpenLanguage((e) => !e)}
          className={`${
            openLanguage ? "bg-primary text-white" : "bg-light-100 text-secondary"
          ...
          {LANGUAGES.map(({ code, name, country_code }) => {
            return (
              <button
                ...
                key={country_code}
                className={`${
                  currentLanguage?.code === code ? "bg-primary text-white" : ""
                } text-left flex items-center gap-2 p-2 hover:bg-primary hover:text-white rounded`}
              >
